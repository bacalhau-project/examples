...
mkdir -p /mnt/data /bacalhau_data/ /bacalhau_data/metadata /cache
...
# --- MOUNT S3FS ---
echo "Mounting s3fs..."
s3fs my-bucket /mnt/data -o url=http://storage:9000 -o use_path_request_style -o passwd_file=/etc/passwd-s3fs -o nonempty  -o allow_other -o use_cache=/cache
...
exec bacalhau serve -c /etc/bacalhau/config.yaml -c "LABELS=PUBLIC_IP=$IP"
