...
def list_resources_with_tag(client, unique_id):
    ...
    for resource in resources:
        if resource.type == 'Microsoft.Compute/virtualMachines' and resource.properties:
            print(f"Resource: {resource.name}, Type: {resource.type}, ID: {resource.id}")
                ...
                for nic in network_profile.get('networkInterfaces', []):
                    ...
                    if nic_id:
                        ...
                        for ip_configuration in ip_configurations:
                            ip_address = ip_configuration['properties'].get('privateIPAddress', None)
                            if ip_address:
...
def get_subscription_id():
    ...
    if not subscription_id:
        ...
        subscriptions_output = run_command(["az", "account", "list", "--output", "json"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Deploy, list, or destroy Azure resources using Bicep templates.")
    ...
    if args.create:
        ...
        client.resource_groups.create_or_update(resource_group_name, {'location': location, 'tags': {'uniqueId': unique_id}})
        print("Resource group created successfully")

        # Deploy control plane
        deploy_bicep_template(resource_group_name, "control_plane.bicep", {
    ...
    if args.list:
        if unique_id:
            ...
            print("Error: UNIQUEID file not found. Cannot list resources without a unique ID.")
        exit(0)
