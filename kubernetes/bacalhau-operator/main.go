...
func init() {
	...
	utilruntime.Must(bacalhauorgv1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       fmt.Sprintf("%s.bacalhau.org", strings.ToLower(utils.RandomString(8))),
	})
	if err != nil {
	...
	if err = (&controllers.BacalhauJobReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
		BCleanupManager: cm,
		Bclient:         bclient,
		EventRecorder:   mgr.GetEventRecorderFor("BacalhauJobController"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "BacalhauJob")
		os.Exit(1)
	}
