...
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "databricks-uploader"))
)
...
class AtomicPipelineManager:
    ...
    def set_pipeline(
        self, pipeline_type: PipelineType, updated_by: str = "pipeline-manager"
    ) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                with sqlite3.connect(self.db_path, timeout=30.0) as conn:
                    ...
                    conn.execute("UPDATE pipeline_config SET is_active = 0")
        ...
        for attempt in range(max_retries):
            try:
                with sqlite3.connect(self.db_path, timeout=30.0) as conn:
                    ...
                    cursor = conn.execute(
                        """
                        SELECT pipeline_type, created_at, created_by, is_active
                        FROM pipeline_config
                        ORDER BY id DESC
                        LIMIT ?
                    """,
...
def set(ctx, pipeline_type):
    ...
    if success:
        ...
        click.echo("❌ Failed to update pipeline", err=True)
        ctx.exit(1)
...
def history(ctx, limit, output_format):
    ...
    if output_format == "json":
        ...
        headers = ["Pipeline Type", "Created At", "Created By", "Active"]
...
def monitor(ctx):
    ...
    try:
        while True:
            ...
            if current_config["created_at"] != last_config["created_at"]:
                click.echo("\n🔄 Pipeline changed!")
                click.echo(f"   From: {last_config['pipeline_type']}")
                click.echo(f"   To: {current_config['pipeline_type']}")
                click.echo(f"   At: {current_config['created_at']}")
