...
def parse_args():
    p = argparse.ArgumentParser(description="SQLite → Databricks uploader") # Updated description
    ...
    p.add_argument("--databricks-host", type=str, help="Databricks workspace host name.")
    p.add_argument("--databricks-http-path", type=str, help="HTTP path for the Databricks SQL warehouse or cluster.")
    p.add_argument("--databricks-token", type=str, help="Databricks personal access token (PAT). Best practice: use DATABRICKS_TOKEN env var.")
    p.add_argument("--databricks-database", type=str, help="Target database name in Databricks.")
    p.add_argument("--databricks-table", type=str, help="Target table name in Databricks (e.g., 'schema.table' or just 'table' if database is set).")
...
def main():
    ...
    databricks_host = args.databricks_host or os.getenv("DATABRICKS_HOST") or cfg.get("databricks_host")
    databricks_http_path = args.databricks_http_path or os.getenv("DATABRICKS_HTTP_PATH") or cfg.get("databricks_http_path")
    databricks_token = args.databricks_token or os.getenv("DATABRICKS_TOKEN") or cfg.get("databricks_token")
    databricks_database = args.databricks_database or os.getenv("DATABRICKS_DATABASE") or cfg.get("databricks_database")
    databricks_table = args.databricks_table or os.getenv("DATABRICKS_TABLE") or cfg.get("databricks_table")
    ...
    if not all([databricks_host, databricks_http_path, databricks_token, databricks_database, databricks_table]):
        missing_params = []
        if not databricks_host: missing_params.append("databricks_host (CLI: --databricks-host, ENV: DATABRICKS_HOST)")
        if not databricks_http_path: missing_params.append("databricks_http_path (CLI: --databricks-http-path, ENV: DATABRICKS_HTTP_PATH)")
        if not databricks_token: missing_params.append("databricks_token (CLI: --databricks-token, ENV: DATABRICKS_TOKEN)")
        if not databricks_database: missing_params.append("databricks_database (CLI: --databricks-database, ENV: DATABRICKS_DATABASE)")
        if not databricks_table: missing_params.append("databricks_table (CLI: --databricks-table, ENV: DATABRICKS_TABLE)")
        logging.error(f"Missing required Databricks connection parameters: {', '.join(missing_params)}")
        sys.exit(1)
    ...
    if args.run_query:
        ...
        try:
            ...
            logging.info(f"Successfully connected to Databricks for query mode: {databricks_host}")
            ...
            def print_query_results(cursor):
                ...
                if cursor.description:
                    ...
                    print("Query executed. No results to display or rowcount not applicable.")

            if args.run_query.strip().upper() == "INFO":
                logging.info(f"Fetching information for Databricks database '{databricks_database}' and table '{databricks_table}'")
                
                # Set current database
                cursor_db_query.execute(f"USE {ParamEscaper.escape_path(databricks_database)}")
                logging.info(f"Using database: {databricks_database}")

                print(f"\n--- Tables in Database '{databricks_database}' ---")
                cursor_db_query.execute(f"SHOW TABLES IN {ParamEscaper.escape_path(databricks_database)}")
                ...
                cursor_db_query.execute(f"DESCRIBE TABLE EXTENDED {qualified_table_name}")
                print_query_results(cursor_db_query)

                print(f"\n--- Row Count for Table '{qualified_table_name}' ---")
                cursor_db_query.execute(f"SELECT COUNT(*) AS row_count FROM {qualified_table_name}")
                print_query_results(cursor_db_query)
                ...
                try:
                    ...
                    logging.warning(f"Could not set database context to '{databricks_database}' (may not be an issue if query is fully qualified): {db_use_exc}")
    ...
    sqlite_table_name   = args.table or os.getenv("SQLITE_TABLE_NAME") or cfg.get("sqlite_table_name") # Renamed for clarity
    timestamp_col= args.timestamp_col or os.getenv("TIMESTAMP_COL") or cfg.get("timestamp_col")
    ...
    if sqlite_table_name is None:
        ...
        cursor_sqlite_meta.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%';")
        ...
        if not tables:
            ...
            sys.exit(1)
        if len(tables) > 1:
            logging.error("Multiple tables found in SQLite database: %s. Please specify one using --table.", tables)
            sys.exit(1)
    ...
    if timestamp_col is None:
        ...
        if not ts_cols:
            logging.error("No suitable timestamp column found in SQLite table '%s'", sqlite_table_name)
            sys.exit(1)
        timestamp_field = ts_cols[0] # Use the first detected suitable column
        logging.info("Detected timestamp field in SQLite table '%s': %s", sqlite_table_name, timestamp_field)
    else:
        timestamp_field = timestamp_col
        logging.info("Using user-supplied timestamp column for SQLite table '%s': %s", sqlite_table_name, timestamp_field)
            ...
            if df.empty:
                ...
                if not df.empty:
                    ...
                    try:
                        ...
                        logging.info(f"Successfully connected to Databricks: {databricks_host}")

                        # Set current database
                        cursor_db.execute(f"USE {ParamEscaper.escape_path(databricks_database)}")
                        logging.info(f"Using database: {databricks_database}")

                        # Prepare for batch insert
                        table_name_qualified = f"{ParamEscaper.escape_path(databricks_database)}.{ParamEscaper.escape_path(databricks_table)}"
                        
                        columns = [ParamEscaper.escape_identifier(col) for col in df.columns]
                        placeholders = ", ".join(["%s"] * len(columns))
                        insert_sql_template = f"INSERT INTO {table_name_qualified} ({', '.join(columns)}) VALUES ({placeholders})"
                        
                        data_to_insert = [tuple(row) for row in df.to_numpy()] # Convert DataFrame rows to tuples
                        
                        batch_size = 500 # Configurable or fixed batch size
                        num_batches = (len(data_to_insert) + batch_size - 1) // batch_size
                        
                        logging.info(f"Preparing to insert {len(data_to_insert)} records in {num_batches} batches of size {batch_size} into {table_name_qualified}...")
                        
                        for i in range(num_batches):
                            batch_data = data_to_insert[i * batch_size : (i + 1) * batch_size]
                            cursor_db.executemany(insert_sql_template, batch_data)
                            logging.info(f"Inserted batch {i+1}/{num_batches} ({len(batch_data)} records).")
                        
                        # conn_db.commit() # Databricks SQL Connector typically autocommits DML like INSERT
                        logging.info(f"Successfully inserted {len(data_to_insert)} new records into Databricks table {table_name_qualified}")
                        upload_successful = True
                    
                    except Exception as e:
                        logging.error(f"Error during Databricks upload: {e}", exc_info=True)
                ...
                if upload_successful:
                    last_ts = df[timestamp_field].max() if not df.empty else last_ts # Keep old last_ts if df is empty
                    with open(state_file, 'w') as f:
                        json.dump({'last_upload': last_ts.isoformat()}, f)
                    logging.info(f"State timestamp updated to: {last_ts.isoformat()}")
                else:
                    logging.warning("Upload to Databricks failed or was incomplete. State timestamp will not be updated.")
