...
class AutoloaderMain:
    ...
    def _validate_configs(self):
        ...
        if missing_files:
            ...
            print(f"Run: uv run -s autoloader_main.py setup")
            sys.exit(1)
    ...
    def setup_infrastructure(self, args) -> int:
        ...
        try:
            ...
            print(f"  uv run -s autoloader_main.py monitor")
        ...
        try:
            ...
            if args.test_type == "all":
                ...
                if args.test_type == "raw":
                    ...
                    tester.test_emergency_detection()
        ...
        try:
            ...
            if 'components' in health:
                if 'databricks' in health['components']:
                    print(f"  📡 Databricks: {health['components']['databricks'].get('status', 'unknown')}")
                if 's3' in health['components']:
                    s3_comp = health['components']['s3']
                    print(f"  🗄️  S3 Buckets: {s3_comp.get('accessible_buckets', 0)}/{s3_comp.get('total_buckets', 0)}")
...
Examples:
  ...
  uv run -s autoloader_main.py status
    ...
    # Global options
    parser.add_argument("--uc-config", default="unity_catalog_config.yaml", 
                       help="Unity Catalog configuration file")
    parser.add_argument("--s3-config", default="s3-uploader-config.yaml",
    ...
    process_parser.add_argument("--continuous", action="store_true",
                               help="Continuous processing")
    process_parser.add_argument("--interval", type=int, default=60,
    ...
    test_parser.add_argument("--type", dest="test_type",
                            choices=["all", "raw", "schematized", "emergency", "s3-access", "partitioning"],
