...
class PipelineLogger:
    ...
    def log_performance(self, operation: str, duration: float, 
                       records: Optional[int] = None, **metadata):
    ...
    def log_exception(self, exception: Exception, context: Optional[Dict[str, Any]] = None):
        ...
        self.error(f"Exception: {type(exception).__name__}: {exception}", 
                  exc_info=True, **exc_data)
...
def run_demo():
    ...
    logger.log_event("emergency_detected", event_type="warning", 
                    sensor="NYC_002", temperature=85.0)
