...
def setup():
    ...
    try:
        ...
        console.print("\n[bold]Creating sensor_data_ingestion table[/bold]")
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS sensor_data_ingestion (
                sensor_id STRING,
                timestamp TIMESTAMP,
                temperature DOUBLE,
                humidity DOUBLE,
        ...
        console.print(f"\nTable: {CATALOG}.{DATABASE}.sensor_data_ingestion")
...
def query():
    ...
    try:
        ...
        cursor.execute(f"""
            ...
            FROM {CATALOG}.{DATABASE}.sensor_data_ingestion
        """)
...
def teardown():
    ...
    if conn:
        cursor = conn.cursor()
        try:
            cursor.execute(f"DELETE FROM {CATALOG}.{DATABASE}.sensor_data_ingestion")
            console.print("\n[green]✓ Deleted all data from sensor_data_ingestion[/green]")
        except Exception as e:
            console.print(f"[red]Error: {e}[/red]")
        finally:
            cursor.close()
