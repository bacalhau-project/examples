...
def query_unity_catalog():
    ...
    schema = os.getenv("DATABRICKS_SCHEMA", "sensor_data")
    database = os.getenv("DATABRICKS_DATABASE", "sensor_data")  # Also check DATABASE
    
    # Use DATABASE if SCHEMA not set
    if schema == "sensor_data" and database:
        schema = database
            ...
            with connection.cursor() as cursor:
                ...
                for table_info in all_tables:
                    ...
                    try:
                        ...
                        # Only query tables with timestamp and turbine_id columns
                ...
                headers = ["Table", "Records", "Earliest", "Latest", "Turbines"]
                    ...
                    for r in results:
                        if r[1] > 0 and "Error" not in str(r[2]):  # Has records and no error
                            union_parts.append(f"""
                                SELECT '{r[0]}' as stage, timestamp, turbine_id 
