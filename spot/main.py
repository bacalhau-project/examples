...
MAX_NODES = 5  # Global limit for total nodes across all regions
KEY_PAIR_NAME = "smt-keypair"

if os.path.exists(KEY_PAIR_NAME + ".pub"):
...
async def create_spot_instances(orchestrators, token=""):
    ...
    async def create_in_region(region):
        ...
        logging.debug(f"Creating spot instance in region: {region}")
...
async def main():
    global table_update_running, INSTANCES_PER_REGION, TOTAL_INSTANCES
    table_update_running = False
    ...
    if args.no_of_instances is not None:
        print("Setting total instances to:", args.no_of_instances)
        TOTAL_INSTANCES = args.no_of_instances
        INSTANCES_PER_REGION = TOTAL_INSTANCES // len(AWS_REGIONS)
