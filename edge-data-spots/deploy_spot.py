...
async def create_spot_instances_in_region(config: Config, instances_to_create, region):
    ...
    try:
        ...
        efs_mount_ip = await create_efs(ec2, efs_client, region, vpc_id)

        user_data = scripts_provider.create_cloud_init_script(efs_mount_ip=efs_mount_ip)
        if not user_data:
...
async def create_efs(ec2, efs_client, region, vpc_id):
    """Creates an EFS if it does not exist, ensures the mount target is available, and returns its IP."""
    try:
        # Check existing EFS
        response = await asyncio.to_thread(efs_client.describe_file_systems)
        existing_efs = [fs for fs in response.get("FileSystems", []) if fs.get("LifeCycleState") == "available"]

        if existing_efs:
            file_system_id = existing_efs[0]["FileSystemId"]
            logging.info(f"Found existing EFS: {file_system_id} in {region}")
        else:
            response = await asyncio.to_thread(efs_client.create_file_system, PerformanceMode='generalPurpose')
            file_system_id = response['FileSystemId']
            logging.info(f"Created EFS: {file_system_id} in {region}")

            # Wait for EFS to become available
            for _ in range(100):
                fs_desc = await asyncio.to_thread(
                    efs_client.describe_file_systems, FileSystemId=file_system_id
                )
                fs_state = fs_desc['FileSystems'][0]['LifeCycleState']
                if fs_state == 'available':
                    logging.info(f"EFS {file_system_id} is available.")
                    break
                await asyncio.sleep(6)
            else:
                logging.error(f"EFS {file_system_id} not available after timeout.")
                return None

        # Check for existing mount target
        targets_resp = await asyncio.to_thread(efs_client.describe_mount_targets, FileSystemId=file_system_id)
        if targets_resp['MountTargets']:
            mt = targets_resp['MountTargets'][0]
            logging.info(f"Found existing mount target for EFS {file_system_id} - State: {mt['LifeCycleState']}")
        else:
            # Create mount target
            subnets = await asyncio.to_thread(ec2.describe_subnets, Filters=[{'Name': 'vpc-id', 'Values': [vpc_id]}])
            ...
            try:
                ...
                logging.info(f"Mount target creation started for EFS {file_system_id}")
            except efs_client.exceptions.MountTargetConflict:
                logging.warning(f"Mount target conflict for EFS {file_system_id}, assuming it already exists.")
            await asyncio.sleep(10)

            # Re-fetch mount targets
            targets_resp = await asyncio.to_thread(efs_client.describe_mount_targets, FileSystemId=file_system_id)
            if not targets_resp['MountTargets']:
                logging.error(f"No mount targets found for EFS {file_system_id} after creation attempt.")
                return None
            mt = targets_resp['MountTargets'][0]

        # Wait for mount target to be available
        for _ in range(100):
            mt_desc = await asyncio.to_thread(
                efs_client.describe_mount_targets, FileSystemId=file_system_id
            )
            mt = mt_desc['MountTargets'][0]
            if mt['LifeCycleState'] == 'available':
                ip = mt['IpAddress']
                logging.info(f"Mount target for EFS {file_system_id} is available. IP: {ip}")
                return ip
            logging.info(f"Waiting for mount target to be available... State: {mt['LifeCycleState']}")
            await asyncio.sleep(6)
        else:
            logging.error(f"Mount target for EFS {file_system_id} not available after timeout.")
            return None

    except Exception as e:
        logging.error(f"Error creating EFS in {region}: {str(e)}")
        return None
