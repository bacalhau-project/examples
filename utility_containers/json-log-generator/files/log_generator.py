...
def generate_log_entry():
    ...
    # Define categories with their weights for networking module
    categories = [
        "[INFO]",
        "[WARN]",
        "[CRITICAL]",
    ...
    # Select module first
    module = choice(module_names)

    # Select category based on module
    if module == "networking":
        category = choices(
            categories, weights=[networking_weights[cat] for cat in categories], k=1
        )[0]
    else:
    ...
    log_entry = {
        ...
        "module": module,
        "message": f"{choice(service_names)} {category} {' '.join(choices(word_list, k=5))}",
    }
